 ...
 */
static int verify_range(const char *path, long start_off, long len,
			char expected)
{
	int fd = open(path, O_RDONLY);
	if (fd < 0) {
	...
	while (remaining > 0) {
		...
		if (nr != to_read) {
			fprintf(stderr,
				"  [FAIL] %s: short read at offset %ld\n", path,
				offset);
			free(buf);
			close(fd);
			return 1;
		...
		for (long i = 0; i < to_read; i++) {
			if (buf[i] != expected) {
				fprintf(stderr,
					"  [FAIL] %s: byte %ld expected '%c' (0x%02x) got 0x%02x\n",
					path, offset + i, expected,
...
static int verify_with_diffs(const char *path, long file_size, char base_fill,
			     const long *diff_pages, int n_diffs,
			     char diff_fill)
{
	int fd = open(path, O_RDONLY);
	if (fd < 0) {
	...
	for (long p = 0; p < total_pages; p++) {
		...
		for (int i = 0; i < PAGE_SIZE; i++) {
			if (buf[i] != expected) {
				fprintf(stderr,
					"  [FAIL] %s: page %ld byte %d expected '%c' got 0x%02x\n",
					path, p, i, expected,
					(unsigned char)buf[i]);
				free(buf);
				close(fd);
...
int main(void)
{
	...
	if (write_file(F_BASE, BIG_FILE_SIZE, 'A') < 0)
		return 1;

	/* exact.dat — identical copy */
	printf("  Creating %s (exact copy)...\n", F_EXACT);
	if (write_file(F_EXACT, BIG_FILE_SIZE, 'A') < 0)
		return 1;

	/* near_miss.dat — 1 page different */
	printf("  Creating %s (page 7 = 'X')...\n", F_NEARMISS);
	if (write_file(F_NEARMISS, BIG_FILE_SIZE, 'A') < 0)
		return 1;
	if (patch_page(F_NEARMISS, 7, 'X') < 0)
		return 1;

	/* scattered.dat — 5 scattered diff pages */
	printf("  Creating %s (pages 50,200,500,800,1500 = 'Y')...\n",
	       F_SCATTERED);
	if (write_file(F_SCATTERED, BIG_FILE_SIZE, 'A') < 0)
		return 1;
	for (int i = 0; i < (int)N_SCATTERED; i++) {
		if (patch_page(F_SCATTERED, scattered_diffs[i], 'Y') < 0)
			return 1;
	}
	...
	if (write_file(F_ALIEN, BIG_FILE_SIZE, 'Z') < 0)
		return 1;

	/* tiny.dat — 16KB of 'A' (4 pages, order-0 folios) */
	printf("  Creating %s (16KB of 'A')...\n", F_TINY);
	if (write_file(F_TINY, TINY_FILE_SIZE, 'A') < 0)
		return 1;
	...
	const char *all_files[] = { F_BASE,	 F_EXACT,    F_NEARMISS,
				    F_SCATTERED, F_HALFDIFF, F_ZEROHEAD,
				    F_ALIEN,	 F_TINY };
	for (int i = 0; i < 8; i++) {
		if (queue(all_files[i]) < 0)
			return 1;
		printf("  [+] Queued: %s\n", all_files[i]);
	}
	for (int i = 0; i < 8; i++)
	...
	/* near_miss.dat — all 'A' except page 7 = 'X' */
	{
		long diff = 7;
		printf("  %s: ", F_NEARMISS);
		if (verify_with_diffs(F_NEARMISS, BIG_FILE_SIZE, 'A', &diff, 1,
				      'X') == 0)
			printf("OK\n");
		else
			fail_count++;
	...
	if (verify_with_diffs(F_SCATTERED, BIG_FILE_SIZE, 'A', scattered_diffs,
			      N_SCATTERED, 'Y') == 0)
		printf("OK\n");
	else
		fail_count++;
	...
	if (patch_page(F_EXACT, 0, 'B') < 0)
		return 1;
	if (patch_page(F_EXACT, 100, 'B') < 0)
		return 1;
	if (patch_page(F_EXACT, 500, 'B') < 0)
		return 1;

	/* Write 'C' to page 0 of near_miss.dat */
	printf("  Writing 'C' to page 0 of %s...\n", F_NEARMISS);
	if (patch_page(F_NEARMISS, 0, 'C') < 0)
		return 1;
	...
	printf("  Verifying %s has COW changes...\n", F_EXACT);
	{
		...
		int cow_ok = 1;
		for (int i = 0; i < 3; i++) {
			if (verify_range(F_EXACT, cow_pages[i] * PAGE_SIZE,
					 PAGE_SIZE, 'B') != 0) {
				cow_ok = 0;
				break;
			}
		...
		if (cow_ok &&
		    verify_range(F_EXACT, 1 * PAGE_SIZE, PAGE_SIZE, 'A') != 0)
			cow_ok = 0;
	...
	{
		int ok = 1;
		if (verify_range(F_NEARMISS, 0, PAGE_SIZE, 'C') != 0)
			ok = 0;
		if (ok && verify_range(F_NEARMISS, 7 * PAGE_SIZE, PAGE_SIZE,
				       'X') != 0)
			ok = 0;
		/* Check page 1 is still 'A' */
		if (ok && verify_range(F_NEARMISS, 1 * PAGE_SIZE, PAGE_SIZE,
				       'A') != 0)
			ok = 0;
		/* Check page 8 is still 'A' */
		if (ok && verify_range(F_NEARMISS, 8 * PAGE_SIZE, PAGE_SIZE,
				       'A') != 0)
			ok = 0;
	...
	if (truncate(F_HALFDIFF, 2 * MB) < 0) {
		...
		struct stat st;
		stat(F_HALFDIFF, &st);
		check(st.st_size == 2 * MB,
		      "half_diff.dat should be 2MB after truncate");
	}
